...
def parse_arguments(parser: ArgumentParser) -> Dict[str, Any]:
    ...
    parser.add_argument('--dataset_source', type=str, default="./dataset_pk/pstm/", help="")
    parser.add_argument('--dataset_source2', type=str, default="./data_paper/data_peking_pred/", help="")
    parser.add_argument('--dataset_roadid_length', type=str, default="./data_paper/data_peking_pred/roadid_length.pkl", help="")
    parser.add_argument('--dataset_dir', type=str, default="./dataset_traj_asnn2/", help="")
    # parse arguments
...
def main(args_dict):
    ...
    if args_dict["mode"] == "train":
        data_train = np.load(args_dict["dataset_source"]+"train.npz")['y'][:,0,:,:]#(2016, 6, 486, 4)
        args_dict["time_num"] = data_train.shape[0]
        args_dict["nodes_num"] = data_train.shape[1]
        seg_embs = np.load("dataset_pk/ebds.npz")["train_ebds"]
    elif args_dict["mode"] == "val":
        data_train = np.load(args_dict["dataset_source"]+"val.npz")['y'][:,0,:,:]
        args_dict["time_num"] = data_train.shape[0]
        args_dict["nodes_num"] = data_train.shape[1]
        seg_embs = np.load("dataset_pk/ebds.npz")["val_ebds"]
    ...
    with open(args_dict["dataset_source2"]+"nodeid_pair_to_roadid_double.pkl","rb") as f:
