...
def load_adj(filename):
    adj = np.loadtxt(filename)#(486, 486)
    return adj
...
def load_dataset(dataset_dir, events_data_dir, batch_size, valid_batch_size=None, test_batch_size=None):
    ...
    data = {}#x_train.shape  (1802, 24, 486, 4) array([65.,  1.,  3.,  1.], dtype=float32)  y_train.shape (1802, 6, 486, 4)
    for category in ['train', 'val', 'test']:#x_test (515, 24, 486, 4)   //y_test.shape  (515, 6, 486, 4) array([97., 57.,  3.,  2.], dtype=float32)
...
def load_dataset_ne(dataset_dir, batch_size, valid_batch_size=None, test_batch_size=None):
    ...
    data = {}#x_train.shape  (1802, 24, 486, 4) array([65.,  1.,  3.,  1.], dtype=float32)  y_train.shape (1802, 6, 486, 4)
    for category in ['train','test']:#x_test (515, 24, 486, 4)   //y_test.shape  (515, 6, 486, 4) array([97., 57.,  3.,  2.], dtype=float32)
        cat_data = np.load(os.path.join(dataset_dir, category + '.npz'))
        data['x_' + category] = cat_data['x'].astype(np.float32)
        data['y_' + category] = cat_data['y'].astype(np.float32)
