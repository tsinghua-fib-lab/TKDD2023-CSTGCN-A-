...
def main():
    ...
    adj_mx = util.load_adj(basepath+"data/adj_mx.txt")
    adj_mx = torch.tensor(adj_mx).to(device).float()
    dataloader = util.load_dataset(basepath+'data/peking/pstm',basepath+'data/peking/em',32,32,32)
    scaler = dataloader['scaler']
    
    supports = None
    expid = 1
    sys.stdout = Logger(save_path + "exp_bj_" + str(expid)+"_output.txt", "a")
    engine = trainer(scaler, 4, 24, 486, 32, 0.3, 0.003, 0.0001, device, supports,adj_mx)
    ...
    for i in range(1, 1000+1):
        ...
        for iter, (x, y, events) in enumerate(dataloader['train_loader'].get_iterator()):
            ...
            metrics = engine.train(trainx, trainy[:, 0, :, :], events)#trainx:torch.Size([64, 4, 486, 24]) trainy:torch.Size([64, 4, 486, 6])
        ...
        seg_embs = []
        for itr in range(18):
            output, seg_emb = engine.model(x[itr*16:(itr+1)*16],x_event[itr*16:(itr+1)*16])
        ...
        if (not os.path.exists(save_path+"TestTime_nasf.npy")) or (early_stopping.counter == 0):
            ...
            seg_embs = np.insert(seg_embs,0,np.zeros([1,24]),1)# (288,417,16) 方便查询
