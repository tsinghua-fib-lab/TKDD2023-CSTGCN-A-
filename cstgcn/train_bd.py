...
def main():
    ...
    adj_mx = util.load_adj(basepath+"data/adj_mx_bd.txt")
    adj_mx = torch.tensor(adj_mx).to(device).float()
    dataloader = util.load_dataset(basepath+'data/peking/pstm_bd',basepath+'data/peking/em_bd',32,16,32)
    scaler = dataloader['scaler']
    
    supports = None
    expid = 5
    sys.stdout = Logger(save_path + "exp_bd_" + str(expid)+"_output.txt", "a")
