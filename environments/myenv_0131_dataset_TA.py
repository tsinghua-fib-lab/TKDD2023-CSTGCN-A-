...
import torch
import torch.nn as nn
from utils.pytorch_models_0131_dataset_TA import Whole_net
...
class GridWorld(Environment):
    ...
    def setPassTimeMatrix(self, passtime_matrix):
        ...
        # self.passtime_matrix = passtime_matrix
    def set_fea_data(self, mean_traffic, seg_embs, node_adj_pad, node_segs_pad,node_segs):
    ...
    def state_to_nnet_input(self, states: List[RFState]) -> List[np.ndarray]:
        ...
        states_np = self.edit_feas(states_np,self.if_use_mf,self.if_add_anode)
    ...
    def get_fea(self,colors):
        if int(colors[1])%288 == 0:
            asegs = [0]
        else:
            asegs = self.node_segs[int(colors[1])] #编号从1开始的0是pad
        
        if int(colors[1])%288 == 0:
            asegs_e = [0]
        else:
            asegs_e = self.node_segs[int(colors[2])]
        ...
        self.fea = np.hstack([as_emb,as_emb_e])
        return self.fea
    ...
    def get_nnet_model(self) -> nn.Module:
        ...
        nnet = Whole_net(out_dim = 64, nodes_num = self.world_nodes, time_num = self.world_time,
                          use_nodeid = self.use_nodeid, if_use_mf = self.if_use_mf, if_add_anode = self.if_add_anode)
        return nnet
