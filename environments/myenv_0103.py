...
class GridWorld(Environment):
    ...
    def generate_states(self, num_states: int) -> Tuple[List[RFState], List[np.ndarray]]:
        ...
        start_time = np.random.randint(0, self.world_time-4, 1).item()
        
        des = np.hstack([np.random.choice(des_list,1)]*num_states)
        cur_seg = np.hstack([nodes_list]*4)[:num_states]
        cur_time = np.repeat(np.array(range(start_time,start_time+4)), len(nodes_list))[:num_states]
