...
import torch
import torch.nn as nn
from utils.pytorch_models_0131_dataset import Whole_net
...
class RFState(State):
    ...
    def get_meantime(self,cur_node,next_node,cur_time,pre_time = 0):
        ...
        # return passtime
        now_time = (cur_time+pre_time)%self.passtime_matrix.shape[0]
        passtime = self.passtime_matrix[now_time, cur_node-1]
        return passtime
    def __hash__(self):
...
class GridWorld(Environment):
    ...
    def __init__(self, time_num, nodes_num):
        ...
        self.if_use_mf = False
        self.if_add_anode = False

    def setStateTransition(self, state_trans,node_connected_list=None):
    ...
    def setPassTimeMatrix(self, passtime_matrix):
        ...
        # self.passtime_matrix = passtime_matrix
    def set_fea_data(self, mean_traffic, seg_embs, node_adj_pad, node_adj_not_pad = None):
        self.mean_traffic = (mean_traffic - mean_traffic.mean())/mean_traffic.std()
        self.seg_embs = seg_embs
        self.node_adj_pad = node_adj_pad
        if node_adj_not_pad != None:
            self.node_adj_not_pad = node_adj_not_pad
    ...
    def state_to_nnet_input(self, states: List[RFState]) -> List[np.ndarray]:
        ...
        # elif self.mode == "val":
        #     states_np = np.hstack([states_np[:,[0]]%288,states_np[:,[1,2]],(states_np[:,[0]]//288)%7+6]) 
        # else:
        #     states_np = np.hstack([states_np[:,[0]]%288,states_np[:,[1,2]],(states_np[:,[0]]//288)%7+5])         

        # states_np = np.hstack([states_np,fea_np])
        # states_np = self.edit_feas(fea_np,self.if_use_mf,self.if_add_anode)
    ...
    def get_length(self):
        ...
        if self.if_use_mf:
            self.length += 1
        if self.if_add_anode:
            self.length += len(self.node_adj_pad[0])*2
        

    def get_fea(self,colors):
        if (int(colors[1])%288 == 0) or (int(colors[2])%288 == 0):
            self.fea = np.array([0]*self.length)
        else:
            asegs = self.node_adj_not_pad[int(colors[1])] #编号从1开始的0是pad
            asegs_e = self.node_adj_not_pad[int(colors[2])]
            
            # as_emb = self.seg_embs[int(colors[0])%288][asegs].mean(axis = 0) #这里的seg_embs(514, 487, 16) 得到5*16
            # as_emb_e = self.seg_embs[int(colors[0])%288][asegs_e].mean(axis = 0)
            origin  = int(colors[1]) if colors[1]<=self.world_nodes else int(asegs[0])
            des = int(colors[2]) if colors[2]<=self.world_nodes else int(asegs_e[0])
            if self.mode == "train":
                state = np.array([colors[0]%288,origin,des,(colors[0]//288)%7+1]) 
            elif self.mode == "val":
                state = np.array([colors[0]%288,origin,des,(colors[0]//288)%7+6])  
            else:
                state = np.array([colors[0]%288,origin,des,(colors[0]//288)%7+5]) 
            as_emb = self.seg_embs[int(colors[0])%288][origin] #这里的seg_embs(514, 487, 16) 得到5*16
            as_emb_e = self.seg_embs[int(colors[0])%288][des]

            self.fea = np.hstack([state,as_emb,as_emb_e]) 

            if self.if_use_mf:
                mf = self.mean_traffic[colors[0].astype(int)]
                self.fea = np.concatenate((self.fea,[mf]))
            if self.if_add_anode:
                # anodes = np.array(itemgetter(*fea_np[:,1].astype(int))(self.node_adj_pad)) #编号从1开始的0是pad
                # anodes_e = np.array(itemgetter(*fea_np[:,2].astype(int))(self.node_adj_pad))
                # fea_np = np.hstack([fea_np,anodes,anodes_e])  
                anodes = np.array(self.node_adj_pad[colors[1]])
                anodes_e = np.array(self.node_adj_pad[colors[2]])
                self.fea = np.concatenate([self.fea,anodes,anodes_e]) 
            if self.fea[1] >= 7000:
                print("this")
        return self.fea

    def edit_feas(self,fea_np, if_use_mf, if_add_anode):
        if if_use_mf:
            self.if_use_mf = True
            mf = self.mean_traffic[fea_np[:,0].astype(int)]
            fea_np = np.hstack([fea_np,mf.reshape(-1,1)])
        if if_add_anode:
            self.if_add_anode = True
            anodes = np.array(itemgetter(*fea_np[:,1].astype(int))(self.node_adj_pad)) #编号从1开始的0是pad
            anodes_e = np.array(itemgetter(*fea_np[:,2].astype(int))(self.node_adj_pad))
            fea_np = np.hstack([fea_np,anodes,anodes_e])
    ...
    def get_nnet_model(self) -> nn.Module:
        # nnet = nn.ModuleList([actor_fleet(out_dim = 64,nodes_num = self.world_nodes,time_num = self.world_time),route_emb_net(64)])
        nnet = Whole_net(out_dim = 64, nodes_num = self.world_nodes, time_num = self.world_time,
                          use_nodeid = self.use_nodeid, if_use_mf = self.if_use_mf, if_add_anode = self.if_add_anode,max_adj_num = self.max_adj_num)
        return nnet
    
    def load_data(self,data):
        self.data = data
        states_nnets = data["states_nnets"]
        self.states_nnets = np.concatenate(states_nnets,axis=0)
        masks_nnets = data["masks_nnets"]
        self.masks_nnets = np.concatenate(masks_nnets,axis=0)
        # astar_labels = data["astar_labels"]
        # astar_seg_embs_not_pads = data["astar_seg_embs_not_pads"]
    ...
    def generate_states(self, num_states: int) -> Tuple[List[RFState], List[np.ndarray]]:
        ...
        start_time = np.random.randint(0, self.world_time-3, 1).item()
        
        des = np.hstack([np.random.choice(des_list,1)]*num_states)
        cur_seg = np.hstack([nodes_list]*5)[:num_states]
        cur_time = np.repeat(np.array(range(start_time,start_time+3)), len(nodes_list))[:num_states]
