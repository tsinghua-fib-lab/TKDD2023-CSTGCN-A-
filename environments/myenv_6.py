...
from utils.pytorch_models import actor_fleet
...
class GridWorld(Environment):
    ...
    def __init__(self, time_num, nodes_num, query):
        self.dtype = np.uint8
    ...
    def setPassTimeMatrix(self, passtime_matrix):
        ...
        self.passtime_matrix = X_nor

        # self.passtime_matrix = passtime_matrix
    def set_fea_data(self, mean_traffic, node_gpsl, seg_embs, node_adj_pad, node_segs_pad):
        self.mean_traffic = mean_traffic
        self.node_gpsl = node_gpsl
    ...
    def get_fea(self,colors):
        ...
        sn = self.node_gpsl[int(colors[1])-1]
        en = self.node_gpsl[int(colors[2])-1]
        dis = geodesic(sn,en).meters/1000
        ...
        self.fea = np.hstack([np.array([dis,mf]),anodes,as_emb])
        return self.fea
    ...
    def generate_states(self, num_states: int) -> Tuple[List[RFState], List[np.ndarray]]:
        ...
        # des = np.random.choice(nodes_list,num_states)
