...
def astar_update(states: List[State],mask_np: np.ndarray ,env: Environment, num_steps: int, heuristic_fn):
    weights: List[float] = list(np.random.rand(len(states)))
    astar = AStar(states, mask_np, env, heuristic_fn, weights)
    for _ in range(num_steps):
        astar.step(heuristic_fn, 1, verbose=False)

    nodes_popped: List[List[Node]] = astar.get_popped_nodes()
    masks_popped: List[List[np.ndarray]] = astar.get_popped_masks()
    nodes_popped_flat: List[Node]
    masks_popped_flat: List[np.ndarray]
    nodes_popped_flat, _ = misc_utils.flatten(nodes_popped)
    masks_popped_flat, _ = misc_utils.flatten(masks_popped)

    for node, mask in zip(nodes_popped_flat, masks_popped_flat):
        node.compute_bellman(mask)############
    ...
    masks_update = masks_popped_flat
    is_solved: np.array = np.array(astar.has_found_goal())

    return states_update, masks_update, cost_to_go_update, is_solved
...
def update_runner(num_states: int, back_max: int, update_batch_size: int, heur_fn_i_q, heur_fn_o_q,
    ...
    while start_idx < num_states:
        ...
        states_itr, mask_np = env.generate_states(end_idx - start_idx)

        if update_method.upper() == "GBFS":
            states_update, cost_to_go_update, is_solved = gbfs_update(states_itr,mask_np, env, num_steps, heuristic_fn, eps_max)
        elif update_method.upper() == "ASTAR":
            states_update, masks_update, cost_to_go_update, is_solved = astar_update(states_itr,mask_np, env, num_steps, heuristic_fn)
        else:
        ...
        masks_update_nnet: List[np.ndaray] = env.mask_to_nnet_input(masks_update)
        result_queue.put((states_update_nnet, masks_update_nnet, cost_to_go_update, is_solved))
...
class Updater:
    ...
    def update(self):
        ...
        states_update_nnet, masks_update_nnet, cost_to_go_update, is_solved = self._update()

        output_update = np.expand_dims(cost_to_go_update, 1)

        return states_update_nnet, masks_update_nnet, output_update, is_solved

    def _update(self) -> Tuple[List[np.ndarray], np.ndarray, np.ndarray]:
        # process results
        states_update_nnet_l: List[List[np.ndarray]] = []
        masks_update_nnet_l: List[List[np.ndarray]] = []
        ...
        while none_count < len(self.procs):
            ...
            masks_nnet_q: List[np.ndarray]
            states_nnet_q, masks_nnet_q, cost_to_go_q, is_solved_q = result
            states_update_nnet_l.append(states_nnet_q)
            masks_update_nnet_l.append(masks_nnet_q)
        ...
        states_update_nnet: List[np.ndarray] = []
        masks_update_nnet: List[np.ndarray] = []
        for np_idx in range(num_states_nnet_np):
            ...
            masks_nnet_idx: np.ndarray = np.concatenate([x[np_idx] for x in masks_update_nnet_l], axis=0)
            masks_update_nnet.append(masks_nnet_idx)
        ...
        return states_update_nnet, masks_update_nnet, cost_to_go_update, is_solved
