...
def add_heuristic_and_cost(nodes: List[Node], scalar, heuristic_fn: Callable,
    ...
    # compute node cost
    # heuristics = heuristic_fn(states,scalar)
...
def bwas_t(args_dict, weight, batch_size, env: Environment, scalar, states: List[State], masks: List[np.ndarray]):
    ...
    start_time = time.time()
    num_itrs: int = 0
    astar = AStar(states, masks, env, scalar, heuristic_fn, weights)
    ...
    while not min(astar.has_found_goal()):#所有实例都完成
        if sum(astar.has_found_goal()) > solved_num and (solved_num%20 == 9):
            ...
            is_end = astar.step(heuristic_fn, batch_size, scalar,verbose = True)
        else:
            is_end = astar.step(heuristic_fn, batch_size, scalar,verbose = False)
        if is_end:
...
def astar_test_nh(args_dict, weight, batch_size, num_states: int, env: Environment, scalar, queries = None):
