...
from utils import nnet_utils_0131_dataset_TA as nnet_utils
...
def add_heuristic_and_cost(nodes: List[Node], scaler, heuristic_fn: Callable,
    ...
    costs: np.ndarray = path_costs + np.array(weights) * heuristics * np.logical_not(is_solved)
...
def bwas_t(args_dict, weight, batch_size, env: Environment, scaler, states: List[State], masks: List[np.ndarray]):
    ...
    while not min(astar.has_found_goal()):#所有实例都完成
        if sum(astar.has_found_goal()) > solved_num and (solved_num%20 == 9):
            ...
            is_end = astar.step(heuristic_fn, batch_size, scaler,verbose = True)
        else:
