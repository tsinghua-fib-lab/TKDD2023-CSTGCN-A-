...
from environments.myenv import RFState
...
def add_heuristic_and_cost(nodes: List[Node], masks: List[np.ndarray], heuristic_fn: Callable,
    ...
    # compute node cost
    heuristics = heuristic_fn(states,masks)
...
class AStar:
    ...
    def __init__(self, states: List[State],masks: List[np.ndarray], env: Environment, heuristic_fn: Callable, weights: List[float]):
        ...
        add_heuristic_and_cost(root_nodes, masks, heuristic_fn, self.weights)
    ...
    def step(self, heuristic_fn: Callable, batch_size: int, include_solved: bool = False, verbose: bool = True):
        ...
        path_costs, heuristics = add_heuristic_and_cost(nodes_c_all_flat, masks_c_all_flat, heuristic_fn, weights)
...
def main():
    ...
    parser.add_argument('--states', type=str, default = "./data/cube3/test/data_0.pkl", help="File containing states to solve")
    ...
    parser.add_argument('--batch_size', type=int, default=20000, help="Batch size for BWAS")
    ...
    parser.add_argument('--results_dir', type=str, default="results/cube3/", help="Directory to save results")
    ...
    #                                                                       "memory.")
    parser.add_argument('--nnet_batch_size', type=int, default=10000, help="Set to control how many states per GPU are "
                                                                          "evaluated by the neural network at a time. "
                                                                          "Does not affect final results, "
                                                                          "but will help if nnet is running out of "
                                                                          "memory.")                                                                    
...
def bwas(model_dir, nnet_batch_size, weight, batch_size, env: Environment, states: List[State], masks: List[np.ndarray]):
    # get device
    on_gpu: bool
    device: torch.device
    ...
    for state_idx, (state, mask) in enumerate(zip(states, masks)):
        start_time = time.time()

        num_itrs: int = 0
        astar = AStar([state], [mask], env, heuristic_fn, [weight])
        while not min(astar.has_found_goal()):
            is_end = astar.step(heuristic_fn, batch_size)
            if is_end:
...
def bwas_t(model_dir, nnet_batch_size, weight, batch_size, env: Environment, states: List[State], masks: List[np.ndarray]):
    # get device
    on_gpu: bool
    device: torch.device
    device, devices, on_gpu = nnet_utils.get_device()
    ...
    start_time = time.time()
    num_itrs: int = 0
    astar = AStar(states, masks, env, heuristic_fn, weights)
    while not min(astar.has_found_goal()):#所有实例都完成
        print("itr_num:",num_itrs)
        print("Num_instance:",len(astar.instances))
        print("Solved Instance Number:",sum(astar.has_found_goal()))
        is_end = astar.step(heuristic_fn, batch_size)
        ...
        if is_end:
            ...
            unsolved = astar.instances[astar.has_found_goal().index(False)].root_node.state.colors
            print("Unsolved States:")
            print(unsolved)
            not_found_num = not_found_num +1
            # break
        else:
...
def astar_test(model_dir, nnet_batch_size, weight, batch_size, num_states: int, env: Environment, queries = None):
    ...
    results: Dict[str, Any] = dict()
    results["states"] = states
    solns, paths, times, num_nodes_gen, found_rate = bwas_t(model_dir, nnet_batch_size, weight, batch_size, env, states, mask_l)
    ...
    for path in results["paths"]:
        ...
        for i in range(len(route)-1):
            ...
            if passed_time>5:
                passed_time = 5
            t1 = env.get_passtime(route[i],route[i+1],start_time,passed_time)*100   
            tims_p += t1
