...
class AStar:
    ...
    def step(self, heuristic_fn: Callable, batch_size: int, scalar, save_dict,include_solved: bool = False, verbose: bool = True):
        ...
        save_dict["queries"] = save_dict["queries"] + [node.state.colors for nodes_c in nodes_c_all for node in nodes_c]
        save_dict["costs"] = save_dict["costs"] + [node.heuristic for nodes_c in nodes_c_all for node in nodes_c]
        check_time = time.time() - start_time
        polist = []
        # for nl in nodes_c_all:
        #     polist = [node.state.colors for node in nl]
...
def bwas_t(args_dict, weight, batch_size, env: Environment, scaler, states: List[State], masks: List[np.ndarray]):
    ...
    heuristic_fn = nnet_utils.load_heuristic_fn_test(args_dict, device, on_gpu, env.get_nnet_model(),
                                                env, clip_zero=True)
    ...
    while not min(astar.has_found_goal()):#所有实例都完成
        if sum(astar.has_found_goal()) > solved_num and (solved_num%20 == 9):
            solved_num = sum(astar.has_found_goal())
            print("itr_num:",num_itrs)
            print("Solved Instance Number:",solved_num)
            is_end = astar.step(heuristic_fn, batch_size, scaler,save_dict,verbose = True)
        else:
            is_end = astar.step(heuristic_fn, batch_size, scaler,save_dict,verbose = False)
        if is_end:
            not_found_num = not_found_num +1
            break
        else:
            num_itrs += 1
    save_dict["queries"] = np.vstack(save_dict["queries"])
    save_dict["costs"] = np.array(save_dict["costs"])
    with open(args_dict['qs']+"/save_dict_w2.pkl","wb") as f:
