...
class AStar():
    def __init__(self,data,state_tran,gps_norm):
...
def basic_label(data,state_tran,test_queries,seg_embs_data,mode):
    gps_norm = 1000

    findpath = AStar(data,state_tran,gps_norm)
    ...
    for query in test_queries:
        ...
        for id in path:
            path_list.append(id)
        for slot in path_timeslot:
            slot_list.append(slot)# 存放当前已经度过的时间片（这里设置的900s为1个时间片）
